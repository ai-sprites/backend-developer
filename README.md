...
```bash
git clone https://github.com/ai-sprites/backend-developer.git
...
```bash
npx --yes --package=github:ai-sprites/backend-developer#v0.1.0 sprite-backend-developer init ./my-project --tool codex
```
...
角色说明位于 [templates/agent.md](templates/agent.md)。实际宿主的发现机制和验证边界见 [工具适配说明](docs/tool-support.md)。在 Claude Code 可用 `claude --agent sprite-backend-developer`；Copilot CLI 可用 `copilot --agent=sprite-backend-developer`；Codex 的原生角色调用与当前安装版本有关；本轮未验证成功，可按工具适配说明显式读取角色指令。
...
- `sprite-backend-developer-data`

只增加一项实践，不影响已有组件：

```bash
node bin/cli.js init ../my-project --tool codex --skills-only --practices sprite-backend-developer-services
```

`--practices all` 选择本包全部实践；逗号列表选择部分。首次 `--skills-only` 不产生角色，之后可以运行普通 `init` 补装。省略实践选项保留已装内容，不解释为删除。

```bash
node bin/cli.js update ../my-project --practices sprite-backend-developer-services
node bin/cli.js remove ../my-project --practices sprite-backend-developer-services
```
...
| 工具 | 角色文件 | 已选实践 |
| --- | --- | --- |
| Codex | `.codex/agents/sprite-backend-developer.toml` | `.agents/skills/<实践>/` |
| Claude | `.claude/agents/sprite-backend-developer.md` | `.claude/skills/<实践>/` |
| Copilot | `.github/agents/sprite-backend-developer.agent.md` | `.github/skills/<实践>/` |

仅生成所选工具的文件，以及 `.sprite/installations/backend-developer.<工具>.json` 中本包的文件归属与版本。不会修改项目的 README、AGENTS.md、CLAUDE.md、应用配置或业务文档。多个角色可按任意顺序安装；重复安装未修改的组件保持原样。
